...
export default function TokenList({ onQuickSwap }: TokenListProps) {
  ...
  return (
    <div className="flex flex-col bg-[#09090b]/90 backdrop-blur-xl border border-white/[0.08] rounded-3xl shadow-2xl overflow-hidden w-full h-[380px] sm:h-[460px] flex-shrink-0">
      {/* Box Header - Aligned with Twitter Feed Frame Header */}
      <div className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3.5 sm:py-4 border-b border-white/[0.07] bg-zinc-950/60 flex-shrink-0">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm sm:text-base font-bold text-zinc-100 flex items-center gap-2">
              ...
              <span className="text-[10px] sm:text-xs font-semibold text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-full font-mono">
          ...
          <p className="text-[11px] sm:text-xs text-zinc-400 font-mono">
            Est. Portfolio Value:{' '}
            <span className="text-zinc-100 font-bold text-xs sm:text-sm">
        ...
        {holdings.length === 0 ? (
          ...
          holdings.map((h) => {
            ...
            return (
              <div
                key={h.address}
                className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-4.5 rounded-2xl border bg-zinc-950/80 border-white/[0.08] hover:border-red-500/30 hover:bg-zinc-950/90 transition-all shadow-md"
              >
                {/* Left: Token Info */}
                <div className="flex items-center gap-3.5">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-zinc-800 to-zinc-950 border border-white/[0.08] flex items-center justify-center text-lg shadow-md flex-shrink-0">
                    {h.icon}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {hasBalance && (
                        <span className="text-[10px] font-semibold text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-full font-mono">
                          Holding
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <button
                        ...
                        className="text-[11px] font-mono text-zinc-500 hover:text-red-400 transition-colors flex items-center gap-0.5 cursor-pointer"
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    {h.symbol !== 'CHEF' && (
                      <button
                        ...
                        className="p-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-red-400 transition-colors cursor-pointer"
        ...
        <form onSubmit={handleImportSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-white/[0.08] rounded-xl px-4 py-2.5 text-xs font-mono text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-red-500/50"
            />
          </div>
